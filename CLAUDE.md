...
MaxMindMinFraudCheckProvider.create()
    ↓
Store result in maxmind_minfraud_check table
```
...
- `performFraudCheck()`: Orchestrates API call, risk evaluation, and action handling
- `handleRiskAction()`: Executes ALLOW/CHALLENGE/BLOCK decisions
- `storeFraudCheck()`: Persists results via provider
...
- Stores: risk_score, decision, raw_response, device_session_id, etc.
- Indexes on user_id, realm_id, timestamp for analytics
